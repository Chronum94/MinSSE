...
template <class IntegerType, class FloatType> struct SimulationInput {
  ...
  SimulationInput(uint16_t _nx, uint16_t _ny, uint16_t _nz, FloatType _beta, FloatType _dilution,
  IntegerType _nbins = 10, IntegerType _msteps = 1000, IntegerType _isteps = 1000) {
    nx = _nx;
    ny = _ny;
    nz = _nz;
    beta = _beta;
  ...
  operator<<(std::ostream &out,
    ...
    out << "========================================\n";
    out << "Lattice: " << in.nx << " x " << in.ny << " x " << in.nz << " = " << in.max_sites
        << " max sites\n";
    out << "Beta: " << in.beta << "\n";
    out << "Dilution: " << in.dilution  << ", exactly " << in.n_active_sites << " active sites.\n";
